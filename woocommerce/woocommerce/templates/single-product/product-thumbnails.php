...
<div class="thumbnails1"><?php
	...
	if ($attachments) {
		...
		foreach ( $attachments as $key => $attachment ) {
			...
			printf( '<a href="%s" title="%s" rel="thumbnails" class="%s">%s</a>', wp_get_attachment_url( $attachment->ID ), esc_attr( $attachment->post_title ), implode(' ', $classes), wp_get_attachment_image( $attachment->ID) );
