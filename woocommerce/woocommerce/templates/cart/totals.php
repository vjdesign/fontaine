				...
				<tr class="total">
					...
					<td><strong><?php
						...
						if (get_option('woocommerce_display_cart_taxes')=='no' && !$woocommerce->cart->prices_include_tax) :
							...
							$f_total = $woocommerce->cart->get_total();
							$f_total = preg_replace( '/[^.0-9]/', '', $f_total );
							$f_total = floatval(substr($f_total,2));
							$f_total = ($f_total + ($_SESSION['sval']));
							echo "$" . $f_total;
							$_SESSION['order_total'] = $f_total;
						endif;
