...
function getQuote1(stt,sub1,pc1,st1,st2,st3,st4)
{
	...
	st1txt = $(st1).text();
	st1 = st1.replace(/<(?:.|\n)*?>/gm, '');
	st1 = st1.replace(st1txt,"");
	//alert(st1);
	if(st1<=40)
	 {
		params = {"FromSuburb":"Hallam","FromState":"VIC","FromPostcode":"3803","DeliveryType":"HDS","ToSuburb":sub1,"ToState":stt,"ToPostcode":pc1,"DoNotPriceAtlJobs":"false","DeliveryItems":[{"ItemDescription":"Vanity Unit","Quantity":"1","DeadWeight":st1,"Depth":st2,"Width":st3,"Height":st4}]};
	 }
	 else
	 {
		params = {"FromSuburb":"Hallam","FromState":"VIC","FromPostcode":"3803","DeliveryType":"HDS","ToSuburb":sub1,"ToState":stt,"ToPostcode":pc1,"DeliveryItems":[{"ItemDescription":"Vanity Unit","Quantity":"1","DeadWeight":st1,"Depth":st2,"Width":st3,"Height":st4}]};
	...
	success: function(xml) 
	{
		 $(xml).find("QuotePrice").each(function()
		{
		 ...
		 if(st1<=40)
		 {
 			document.getElementById('freightp').innerHTML = "$" + t1 + "<span id='atlservice' style='cursor:pointer;' onclick='atlservice()'>*ATL Service, please see conditions</span>";
			document.addtocart.hdsatl.value = "ATL Service";
		 }
		 else
		 {
...
<script>
	function atlservice()
	{
		var html1 = "\n'Authority To Leave' -  Leave Safe Home Delivery (ATL service)\nIn utilising this service, you are acknowledging the carriers right to leave the goods at your location without obtaining a POD signature. You are authorising the driver to leave the goods in a safe area if you are not home. Where the item\s are left will be at the discretion of the driver. If you happen to be home at the time of delivery a signature may be obtained by the driver.  You cannot request the driver to ring you prior to delivery. Your freight is fully insured whilst in transit for loss or damage. If your consignment is lost or damaged after confirmation of leave safe ('Driver POD' or system 'Delivered' status) then your insurance coverage is void.\n\nIf you require a standard delivery service please contact our office.";
		alert(html1);
  	}
</script>
