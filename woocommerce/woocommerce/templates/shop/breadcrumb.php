...
global $post, $wp_query;
 $delimiter = "<div style='display:inline;height:23px;float:left;'><img src='" . get_bloginfo('template_directory') . "/images/breadcrumb_divider.jpg' /> </div>";
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		echo $before . "<span>" . $queried_object->name . "</span>" . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . "<span>" . __('Search results for &ldquo;', 'woocommerce') . get_search_query() . '&rdquo;' . "</span>" . $after;
			...
			echo $before . "<span>" . $_name  . "</span>" . $after;
		...
		echo $before . "<span>" . __( 'Search results for &ldquo;', 'woocommerce' ) . get_search_query() . '&rdquo;' . "</span>" . $after;
	...
	if ( get_query_var( 'paged' ) )
		echo '<span>(' . __( 'Page', 'woocommerce' ) . ' ' . get_query_var( 'paged' ) . ')</span>';
