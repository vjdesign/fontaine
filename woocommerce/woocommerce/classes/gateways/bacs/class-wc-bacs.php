...
class WC_BACS extends WC_Payment_Gateway {
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			...
			'bsbn' => array(
							'title' => __( 'BSB Number', 'woocommerce' ),
							'type' => 'text',
							'description' => '',
							'default' => ''
						),
    ...
    function thankyou_page() {
		...
		?><h3 class="dashed-line"><?php _e('Fontaine Bank Account Details', 'woocommerce') ?></h3>

		
<div style="font-weight:bold;"><?php echo wpautop( wptexturize( $description ) );?></div>
		
		<?php

		$fields = apply_filters('woocommerce_bacs_fields', array(
			'bank_name'		=> __('Bank Name', 'woocommerce'),
			'account_name' 	=> __('Bank Account Name', 'woocommerce'),
			'account_number'=> __('Account Number', 'woocommerce'),
			'bsbn'			=> __('BSB Number', 'woocommerce')
		));


		echo '<div id="bankdetail"><p>';
		foreach ($fields as $key=>$value) :
		    if(!empty($this->$key)) :
		    	echo '<span style="color:#1F97B0;">'.$value.':</span> '.wptexturize($this->$key).'<br/>';
		    endif;
		endforeach;
		echo '<span style="color:#1F97B0;">Reference:</span> <i>Your Order Number</i></p></div>';
		?><br/><?php
    }
