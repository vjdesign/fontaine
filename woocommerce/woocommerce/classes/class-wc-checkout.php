	...
	function __construct () {
		...
		$this->checkout_fields['order']	= array(
			'order_comments' => array(
				...
				'placeholder' => _x('Enter your order notes here.', 'placeholder', 'woocommerce')
				)
			);
	...
	function process_checkout() {
		...
		if ( ! isset( $_POST['woocommerce_checkout_update_totals'] ) && $woocommerce->error_count() == 0 ) :
			...
			while (1) :
				...
				{
					$_SESSION['pickup_shipname'] = "Pickup From:" . "\n" . str_replace("~","\n",$_SESSION['pickup_shipname']);
					$order_data = array(
				...
				{
					$_SESSION['delivery_warehouse'] = "Delivery From:" . "\n" . str_replace("~","\n",$_SESSION['delivery_warehouse']);
					$order_data = array(
						'post_type' 	=> 'shop_order',
						'post_title' 	=> sprintf( __( 'Order &ndash; %s', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'woocommerce' ) ) ),
						'post_status' 	=> 'publish',
				...
				{
					$_SESSION['BigPost_address'] = "Pickup From :" . "\n" . str_replace("~","\n",$_SESSION['BigPost_address']);
					$order_data = array(
						'post_type' 	=> 'shop_order',
						'post_title' 	=> sprintf( __( 'Order &ndash; %s', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'woocommerce' ) ) ),
						'post_status' 	=> 'publish',
				...
				if($_SESSION['pickUp']=="on")
				{
					...
					update_post_meta( $order_id, '_shipping_method', 'local_pickup');

				}
				if($_SESSION['Delivery']=="on")
				{
					$shipping_method = "Local Delivery";
					update_post_meta( $order_id, '_shipping_method', 'local_delivery');

				}
				if($_SESSION['BigPostDB']=="on")
				{
					$shipping_method = "Depot Pickup";
					update_post_meta( $order_id, '_shipping_method', 'local_pickup');

				}

				if(isset($_SESSION['hdsatl'])==true)
				{
					update_post_meta( $order_id, __( 'ATL Service', 'woocommerce' ), $_SESSION['hdsatl'] );
				}
...
//				update_post_meta( $order_id, '_order_shipping', 		woocommerce_format_total( $woocommerce->cart->shipping_total ) );
				update_post_meta( $order_id, '_order_shipping', 		$_SESSION['sval']);
...
//				update_post_meta( $order_id, '_order_shipping_tax', 	$_SESSION['sval']);
//				update_post_meta( $order_id, '_order_total', 			woocommerce_format_total( $woocommerce->cart->total ) );
				update_post_meta( $order_id, '_order_total', 			$_SESSION['order_total'] );
