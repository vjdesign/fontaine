...
class WC_Product {
	...
	function get_price_html_from_to( $from, $to ) {
		return '<del> <span class="amount">was: ' . ((is_numeric($from)) ? woocommerce_price( $from ) : $from) . '</span></del> <ins><span class="amount"> now ' . ((is_numeric($to)) ? woocommerce_price( $to ) : $to) . '</span></ins>';
	}
