...
function woocommerce_update_cart_action() {
	...
	if ( isset($_GET['remove_item']) && $_GET['remove_item'] && $woocommerce->verify_nonce('cart', '_GET')) {
		...
		{
			$_SESSION['pickup_ship'] = $_GET['pickup_ship'];
			$_SESSION['sval'] = $_SESSION['sval'] - ( (int) $_GET['rval'] );
			$_SESSION['pickup_shipname'] = $_GET['new_pickup_shipname'];
		}

		if($_SESSION['Delivery']=="on")
		{
			$_SESSION['ship_delivery'] = $_GET['pickup_ship'];
			$_SESSION['sval'] = $_SESSION['sval'] - ( (int) $_GET['rval'] );
			$_SESSION['delivery_warehouse'] = $_GET['new_pickup_shipname'];
		}

		if($_SESSION['BigPostDB']=="on")
		{
			$_SESSION['ship_bigdp'] = $_GET['pickup_ship'];
			$_SESSION['sval'] = $_SESSION['sval'] - ( (int) $_GET['rval'] );
			$_SESSION['BigPostDB_shipname'] = $_GET['new_pickup_shipname'];
		}
...
//			{
				...
				if($delivery1=="on")
				{
					...
					$ship_delivery = (float) $_REQUEST['freightprice'];
				...
				if($bigpostdp=="on")
				{
					...
					{
						$ship_bigdp = (float) ($_REQUEST['freightprice']);
					}
